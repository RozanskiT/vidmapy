...
class Synthe:
    ...
    def _compute_spectrum(self, tmpdirname, model):
        ...
        self._run_rline2(tmpdirname, model)
        ...
        self._run_syntoascanga(tmpdirname, model)

        return Spectrum.from_synthe_spectrum(self.parameters, tmpdirname)

    def _run_xnfpelsyn(self, tmpdirname, model):
        # Prepare lines data
        os.symlink(os.path.join(self.kurucz_bin_path,"lines","he1tables.dat"), os.path.join(tmpdirname,"fort.18"))
        os.symlink(os.path.join(self.kurucz_bin_path,"lines","molecules.dat"), os.path.join(tmpdirname,"fort.2"))
        os.symlink(os.path.join(self.kurucz_bin_path,"lines","continua.dat"), os.path.join(tmpdirname,"fort.17"))

        self._call_external_code(tmpdirname,
                                os.path.join(self.kurucz_bin_path, "bin", "xnfpelsyn.exe"), 
                                self._extend_model_for_SYNTHE(model)
                                )

    def _run_synbeg(self, tmpdirname, model):
        self._call_external_code(tmpdirname, 
                                os.path.join(self.kurucz_bin_path, "bin", "synbeg.exe"), 
                                self._get_synbeg_input(model)
                                )

    def _run_rline2(self, tmpdirname, model):
        # os.symlink(os.path.join(self.kurucz_bin_path,"lines","gf0600.100"), os.path.join(tmpdirname,"fort.11"))
        os.symlink(os.path.join(self.kurucz_bin_path,"lines","gfall08oct17.dat"), os.path.join(tmpdirname,"fort.11"))
        self._call_external_code(tmpdirname,
                                os.path.join(self.kurucz_bin_path, "bin", "rline2.exe")
                                )
        os.remove(os.path.join(tmpdirname,"fort.11"))

    def _run_rmolecasc(self, tmpdirname, model, molecule_file="coax.dat"):
        os.symlink(os.path.join(self.kurucz_bin_path, "molecules", molecule_file), os.path.join(tmpdirname, "fort.11"))
        self._call_external_code(tmpdirname,
                        os.path.join(self.kurucz_bin_path, "bin", "rmolecasc.exe")
                        )
        os.remove(os.path.join(tmpdirname,"fort.11"))
    ...
    def _run_synthe(self, tmpdirname, model):
        self._call_external_code(tmpdirname,
                                os.path.join(self.kurucz_bin_path, "bin", "synthe.exe")
                                )
    ...
    def _run_spectrv(self, tmpdirname, model):
        ...
        self._call_external_code(tmpdirname,
                                os.path.join(self.kurucz_bin_path, "bin", "spectrv.exe")
                                )      
    ...
    def _run_rotate(self, tmpdirname, model):
        self._call_external_code(tmpdirname,
                                os.path.join(self.kurucz_bin_path, "bin", "rotate.exe"),
    ...
    def _run_broaden(self, tmpdirname, model):
        ...
        self._call_external_code(tmpdirname,
                                os.path.join(self.kurucz_bin_path, "bin", "broaden.exe"),
    ...
    def _run_syntoascanga(self, tmpdirname, model):
        ...
        self._call_external_code(tmpdirname,
                                os.path.join(self.kurucz_bin_path, "bin", "syntoascanga.exe")
                                )
